...
<div id="main" style="font-size: 10pt">
	  ...
	  function kosongkan(){
	    ...
	    document.getElementById('nm_bag').focus();
	 	}  

	 	function caribag(page_number, search){
		  $(this).html("ketik pencarian").attr("disabled", "disabled");
		  
		  $.ajax({
		    url: 'm_bagian_cari.php',
		    type: 'POST',
		    data: {keyword: $("#keyktbag").val(), page: page_number, search: search}, 
		    dataType: "json",
    ...
    <div class="w3-container w3-card" style="background: linear-gradient(165deg, magenta 0%, yellow 45%, white 85%);position: sticky;top:44px;margin-top: -6px;z-index: 1;">
    	<i class='fa fa-briefcase' style="font-size: 18px">&nbsp;MASTER DATA &nbsp;</i> <i class='fa fa-angle-double-right'></i>&nbsp;<span style="font-size: 18px">Bagian Penjualan</span>
    </div>
    <div class="w3-row" style="background: linear-gradient(565deg, #FAFAD2 30%, white 100%);">
      	<!-- #F0E68C -->
      <div class="col-sm-12 w3-card-4">
    	  <form id="form1" class="w3-container" action="m_bagian_act.php" method="post">
    	  	<div class="row w3-margin-top">
    	  		<div class="col-sm-6">
    	  		    <div class="form-group row">
                        <input type="hidden" name="no_urut"  id="no_urut">
    	   	            <label for="nm_bag " class="col-sm-4  col-form-label"><b>Nama Bagian</b></label>
    	   	            <div class="col-sm-8">
    	   	               <input class="form-control hrf_arial" id="nm_bag" type="text" name="nm_bag" autofocus required style="border: 1px solid black;font-size: 10pt;">
    	   	            </div>
      	   	        </div>	
    	  		</div>
    	  	</div>
          ...
          <div class="w3-row">
              ...
              <div class="input-group" style="margin-top: 15px">
	 	  	        <input onkeyup="if(event.keyCode==13){caribag(1, true);}" style="font-size: 10pt;height: 30px" type="text" class="form-control hrf_arial" placeholder="ketik pencarian [keterangan nama satuan]" id="keyktbag">&nbsp;
                <span class="input-group-btn w3-margin-bottom">
                  <button onclick="caribag(1, true);" class="btn btn-primary" type="button" id="btn-ktbag" style="font-size: 10pt;" title="Cari"><i class="fa fa-search"></i></button>
                  <a style="font-size: 10pt;" title="Reset cari" onclick="document.getElementById('keyktbag').value='';document.getElementById('btn-ktbag').click();" href="#" class="btn btn-warning"><i class="fa fa-undo"></i></a>
                </span>
              </div>		
          	</div>
          </div>	
        </div>  
        <div class="hrf_arial" id="viewdtbag" style="margin-top: 0px;"><script>caribag(1,true)</script></div>
	    </div>  
