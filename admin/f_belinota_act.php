<?php 
  ...
  if(mysqli_num_rows($ceknota1)>=1){
    ...
    $kons=opendtcek();
    $d=mysqli_query($kons,"UPDATE beli_brg SET kd_bar='$kd_bar',kd_sup='$kd_sup',kd_sat='$kd_sat',hrg_beli='$hrg_beli',disc1='$discitem1',disc2='$discitem2',jml_brg='$jml_brg',kd_sup='$kd_sup',id_bag='$id_bag',expdate='$expdate' WHERE no_urut='$no_urutnota'");
    $d=mysqli_query($kons,"UPDATE beli_brg SET id_bag='$id_bag' WHERE kd_brg='$kd_brg' AND kd_toko='$kd_toko' AND ket='PEMBELIAN BARANG'");
      ...
      if($adamutasi==0){
        ...
        $d=mysqli_query($kons,"UPDATE beli_brg SET ket='$ketbel',id_bag='$id_bag',ket='$ketbel' WHERE no_urut='$no_urutnota' AND kd_toko='$kd_toko'");  
      }
    $edit=1;
    $rek_end=$no_urutnota;
    mysqli_close($kons);
    ...
    $tgl_tran=date("Y-m-d");
    if($ket=='TUNAI'){
      mysqli_query($connect,"UPDATE beli_bay set tgl_tran='$tgl_tran',tot_beli='$gtot',saldo_awal='$gtot',byr_hutang='$gtot',saldo_hutang='0',pay='0',kd_sup='$kd_sup',ketbeli='$ketbel' WHERE kd_toko='$kd_toko' AND no_fak='$no_fak' AND tgl_fak='$tgl_fak'");
    ...
    if (empty($ketbel)){
      $d=mysqli_query($connect,"INSERT INTO beli_brg values('','$tgl_fak','$no_fak','$kd_brg','$kd_bar','$kd_toko','$kd_sup','$kd_sat','$hrg_beli','$discitem1','$discitem2','$jml_brg','$jml_brgkov','PEMBELIAN BARANG','','','','','$id_bag','$expdate')");
    } else {
      $d=mysqli_query($connect,"INSERT INTO beli_brg values('','$tgl_fak','$no_fak','$kd_brg','$kd_bar','$kd_toko','$kd_sup','$kd_sat','$hrg_beli','$discitem1','$discitem2','$jml_brg','$jml_brgkov','$ketbel','','','','','$id_bag','$expdate')");
    }
    
    // update beli brg
     $d=mysqli_query($connect,"UPDATE beli_brg SET id_bag='$id_bag',kd_bar='$kd_bar' WHERE kd_brg='$kd_brg' AND kd_toko='$kd_toko'");
  ...
  //update keterangan seluruh input
  mysqli_query($connect,"UPDATE beli_brg SET ket='$ketbel' WHERE no_fak='$no_fak' AND tgl_fak='$tgl_fak' AND kd_toko='$kd_toko'");
  $rr=mysqli_query($connect,"SELECT * FROM beli_bay WHERE no_fak='$no_fak' AND tgl_fak='$tgl_fak' AND kd_toko='$kd_toko'");
  if(mysqli_num_rows($rr)>0){
    mysqli_query($connect,"UPDATE beli_bay SET ketbeli='$ketbel' WHERE no_fak='$no_fak' AND tgl_fak='$tgl_fak' AND kd_toko='$kd_toko'");
  }
  unset($ceknota1,$rakhir,$dataku,$rr);
  mysqli_close($connect);
