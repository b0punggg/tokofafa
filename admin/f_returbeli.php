...
<div id="main" style="font-size: 10pt;background: linear-gradient(180deg, #FAFAD2 10%, white 70%)">
  ...
  <body >
     <script>
      ...
      function caribrgretur(page_number, search){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#listbrgretur").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal memuat list retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal memuat list retur pembelian");
          }
        });
      }
      ...
      function cariidsup(page_number, search){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewidsup").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal memuat supplier:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal memuat data supplier");
          }
        });
      }
      ...
      function cariidbrgretur(page_number, search){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewkdbrgretur").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal memuat barang retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal memuat data barang retur");
          }
        });
      }
      ...
      function carisatretur(){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewsatretur").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal memuat satuan retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal memuat data satuan");
          }
        });
      } 
      ...
      function hapusreturbrg(nourut){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewuni").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal hapus item retur:", xhr.status, xhr.responseText);
            }
      ...
      function cekjmlstokbeli(nourut,kdbrg,kdsat){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewjmlstok").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal cek stok retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal mengecek stok barang");
          }
        });
      } 
      ...
      function prosessimpan(notran,tgltran,kembali){
        ...
        $.ajax({
          ...
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal simpan retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal menyimpan retur pembelian");
          }
        });
      } 
      ...
      function prosesbatal(notran,tgltran){
        ...
        $.ajax({
          ...
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal batal retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal membatalkan retur pembelian");
          }
        });
      } 
      ...
      function databaru(notran){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewuni1").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal buat data baru retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal membuat data retur baru");
          }
        });
      } 
      ...
      function carinoretur(page_number, search){
        ...
        $.ajax({
          ...
          success: function(response){ 
            $("#viewcarinoretur").html(getAjaxHtml(response));
          },
          error: function (xhr, ajaxOptions, thrownError) { // Ketika terjadi error
            if (window.console) {
              console.error("Gagal memuat daftar no retur:", xhr.status, xhr.responseText);
            }
            popnew_error("Gagal memuat daftar retur");
          }
        });
      }  
      function kosongkan(){
    ...
    <div class="w3-container w3-card" style="background: linear-gradient(165deg, magenta 0%, yellow 45%, white 85%);position: sticky;top:44px;margin-top: -6px;z-index: 1;">
      <i class='fa fa-cart-arrow-down' style="font-size: 18px">&nbsp;TRANSAKSI &nbsp;</i> <i class='fa fa-angle-double-right'></i>&nbsp;<span style="font-size: 16px">Retur Pembelian Barang</span><span class="w3-right" style="font-size: 16px"><i class="fa fa-calendar-check-o"></i>&nbsp;<?=gantitgl($_SESSION['tgl_set']);?></span>
    </div>
    ...
    <div class="" style="background: linear-gradient(0deg, #FAFAD2 0%,white 30%, white 100%);margin-top: -12px">
      <div id="listbrgretur" style="border-style: ridge;border-color: white"><script>caribrgretur(1,true);</script></div>
      <div id="viewuni"></div>
      <div id="viewuni1"></div>
    </div>
  <br>
  <div class=" w3-row w3-container " style="font-size: 9pt;">
    ...
    <div class="w3-col l2 m12 s12" style="font-size: 9pt;font-weight:bold">
      ...
      <button class="btn-primary form-control fa fa-save w3-hover-shadow w3-margin-bottom" style="font-size: 10pt" onclick="cekinput();">&nbsp;Simpan
          </button>   
      <button class="btn-danger form-control fa fa-trash w3-hover-shadow w3-margin-bottom" style="font-size: 10pt" onclick="batal();carinoretur(1,true);">&nbsp;Batal</button>       
    </div>
    <div class="w3-col l1 s12 m12">
    </div>
    <div class="w3-right w3-col l6 s12 m12 ">
