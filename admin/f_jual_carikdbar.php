    ...
    if (!empty($kd_bar)) {  
      ...
      if(mysqli_num_rows($cekot)>=1){
       ...
       //-------------------------
        ...
        // Cek dan apply discount promo otomatis
        $disc_promo = getdiscpromoitem($kd_brg, $tgl_jual, $conkdbar);
        if ($disc_promo !== false) {
          $disc_rp_promo = floatval($disc_promo['disc_rupiah']);
          $disc_pr_promo = floatval($disc_promo['disc_persen']);
          
          // Jika ada discount persen, hitung dari harga jual
          if ($disc_pr_promo > 0) {
            $discitem = ($hrg_jual * $disc_pr_promo / 100) + $disc_rp_promo;
          } else {
            $discitem = $disc_rp_promo;
          }
        } else {
          // Pastikan discitem sudah diinisialisasi
          if (!isset($discitem)) {
            $discitem = 0.00;
          }
        }
       ...
       //------
        ...
        if ($jumlahstok>=1){ 
          ...
          if ($potong==0){
            ...
            $jumm         = $dtbrg['jml_brg'];
            $klrm         = $dtbrg['brg_klr'];
            $jml_brg_mas  = $jumm-($jml_brg);
            $brg_klr_mas  = $klrm+($jml_brg);
