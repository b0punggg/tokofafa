...
<div class="table-responsive" style="overflow-y:auto;overflow-x: auto;border-style: ridge;">
	  <table class="arrow-nav2 table-bordered table-striped table-hover" style="font-size:8pt;width: 100%">
	    ...
	    if(isset($_POST['search']) && $_POST['search'] == true){ // Jika ada data search yg 
          ...
          if ($params=="") {	 
	          ...
	          $sql2 = mysqli_query($connect, "SELECT COUNT(DISTINCT kd_brg) AS jumlah FROM beli_brg WHERE beli_brg.kd_toko='$kd_toko'");
          }
          else {
	          $sql1 =mysqli_query($connect, "SELECT beli_brg.kd_brg,beli_brg.kd_toko,beli_brg.stok_jual,mas_brg.no_urut,mas_brg.nm_brg,mas_brg.jml_brg,mas_brg.kd_bar,mas_brg.kd_kem1,mas_brg.jum_kem1,mas_brg.hrg_jum1,mas_brg.kd_kem2,mas_brg.jum_kem2,mas_brg.hrg_jum2,mas_brg.kd_kem3,mas_brg.jum_kem3,mas_brg.hrg_jum3,mas_brg.brg_msk,mas_brg.brg_klr,mas_brg.kd_toko 
	            FROM beli_brg LEFT JOIN mas_brg ON beli_brg.kd_brg=mas_brg.kd_brg
	          	WHERE beli_brg.kd_toko='$kd_toko' AND mas_brg.nm_brg LIKE '$param' 
	          	GROUP BY beli_brg.kd_brg 
	          	ORDER BY mas_brg.kd_brg ASC LIMIT $limit_start, $limit");
	    ...
	    while($databrg = mysqli_fetch_array($sql1)){ // Ambil semua data dari hasil eksekusi $sql
	      $no++;
	      // $nm_kem1=ceknmkem(mysqli_escape_string($connect,$databrg['kd_kem1']));
	      // $nm_kem2=ceknmkem(mysqli_escape_string($connect,$databrg['kd_kem2']));
	      // $nm_kem3=ceknmkem(mysqli_escape_string($connect,$databrg['kd_kem3']));
	        ...
	        <td>
	          	<button id="<?='btnnmbrg'.$no?>" onclick="
	               document.getElementById('kd_brg').value='<?=mysqli_escape_string($connect,$databrg['kd_brg']) ?>';
	          	   document.getElementById('nm_brg').value='<?=mysqli_escape_string($connect,$databrg['nm_brg']) ?>';document.getElementById('boxnmbrgp').style.display='none';document.getElementById('nm_sat').focus();
	               " class="yz-theme-d2 fa fa-edit btn btn-sm" type="button" style="cursor: pointer;font-size: 12pt" title="Edit Data">
	            </button>
