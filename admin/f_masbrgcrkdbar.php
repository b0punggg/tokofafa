<?php
    ...
    $no=$jml=0;
    $connect=opendtcek();
    $cek=mysqli_query($connect,"SELECT count(*) as jmlbarkode from mas_brg WHERE mas_brg.kd_bar='$kd_bar' group by mas_brg.kd_bar");
    ...
    if ($jml>1){ ?>	
      <script>document.getElementById('modalkdbarmas').style.display='block';</script> <?php
    }else{
	    ...
	    if (mysqli_num_rows($sql1)>=1) {
	        ...
	        <script>
		  	   ...
		  	   document.getElementById('saycode').innerHTML='<?=mysqli_escape_string($connect,$databrg['kd_bar'])?>';carihrgbeli(1,true);
	        </script>
	      <?php 	 	
	    }else{ ?>
         <script>kosongkan1()</script> <?php
	    } 
    } 		
?>
<script>document.getElementById('kd_brg').focus();</script>
  <div id="modalkdbarmas" class="w3-modal" style="padding-top:50px;background-color:rgba(1, 1, 1, 0.3);border-style: ridge; ">
    <div class="w3-modal-content w3-card-4 w3-animate-top" style="max-width:700px;border-radius:5px;box-shadow: 0px 2px 60px;border-style: ridge;border-color:white">
        ...
        <div class="w3-center">
          <span onclick="document.getElementById('modalkdbarmas').style.display='none'" class="w3-display-topright" title="Close Modal" style="margin-top: -3px;margin-right: 0px"><img style="width: 108%" src="img/tomexit2.png" alt=""></span>    
        </div>
            ...
            <div class="table-responsive hrf_arial" style="overflow-y:auto;overflow-x: auto;border-style: ridge;">
			  <table class="table table-bordered table-sm table-hover" style="font-size:9pt;">
			    <tr align="middle" class="yz-theme-l1">
		          ...
		          <?php  
		            $no=0;
		            $cek=mysqli_query($connect,"SELECT mas_brg.kd_brg,mas_brg.nm_brg,mas_brg.kd_bar,mas_brg.kd_kem1,mas_brg.kd_kem2,mas_brg.kd_kem3,mas_brg.jum_kem1,mas_brg.jum_kem2,mas_brg.jum_kem3,mas_brg.hrg_jum1,mas_brg.hrg_jum2,mas_brg.hrg_jum3 FROM mas_brg WHERE mas_brg.kd_bar='$kd_bar' ");
	            	  ...
	            	  <tr>
	                     ...
	                     <td>
				          	<button onclick="document.getElementById('kd_brg').value='<?=mysqli_escape_string($connect,$data['kd_brg']) ?>';
						  	   ...
						  	   document.getElementById('kd_sat2').value='<?=mysqli_escape_string($connect,$data['kd_kem2']) ?>';document.getElementById('nm_sat2').value='<?=$nm_kem2?>';document.getElementById('jum_sat2').value='<?=mysqli_escape_string($connect,$data['jum_kem2']) ?>';document.getElementById('hrg_jum2').value='<?=gantitides($data['hrg_jum2']) ?>';
						  	   ...
						  	   document.getElementById('lanjutsavemas').value='lanjut';
						  	   document.getElementById('modalkdbarmas').style.display='none';
						  	   document.getElementById('boxidbrg').style.display='none';carihrgbeli(1,true)";
