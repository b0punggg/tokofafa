  ...
  <table class="arrow-navsat table-hover" style="font-size:9pt;width: 100%">
    ...
    ?>
      <tr style="background-color: white">
        ...
        <td>
          <input type="button" class="btn yz-theme-d2"
          ...
          onclick="document.getElementById('kd_sat').value='<?=mysqli_escape_string($connect,$data['kd_kem1']) ?>';
               ...
               document.getElementById('tabkempak').style.display='none';
      ...
      <tr>
        <td align="left"><input class="w3-input" type="text" onkeydown="if(event.keyCode==13){document.getElementById('<?='tmb3'.$no?>').click();}" value="<?php echo $satkecil3; ?>" readonly tabindex='8' id="<?='nm_satu3'.$no?>" style="border: none;outline: none;"></td>
    ...
    if (!empty($param)){
      if ($no>=1){
        $x=explode(';',carisatkecil($param,$kd_toko));
        $kd_satkecil=$x[0];
        ?><script>
