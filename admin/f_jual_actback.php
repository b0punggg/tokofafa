    ...
    //cek data apakah edit data 
      ...
      if (mysqli_num_rows($cekedit)>=1 ){
        ...
        //kembalikan nilai pada mas_brg
          ...
          $a_brg_klr = $dtmas['brg_klr']-$jml_stokkov;
          $a_jml_brg = $dtmas['jml_brg']+$jml_stokkov;
          ...
          mysqli_free_result($sqlmas);unset($dtmas,$a_brg_klr,$a_jml_brg,$a_no_urut);  
      ...
      if ($potong==0){
       ...
       //end transaksi $potong=0
        ...
        if ($stok>0 && $qty_brg*$jum_kem<=$stok){
          ...
          $jml_brg_mas = $dc['jml_brg']-($qty_brg*$jum_kem);
          $brg_klr_mas = $dc['brg_klr']+($qty_brg*$jum_kem);
          ...
          // $conmasbrg=opendtcek();
          // $cekbrg      = mysqli_query($conmasbrg,"SELECT no_urut,jml_brg,brg_klr,nm_brg FROM mas_brg WHERE kd_brg='$kd_brg'");
          // $cx=mysqli_fetch_assoc($cekbrg);
          // $jml_brg_u = $cx['jml_brg'];
          // $brg_klr_u = $cx['brg_klr']; 
          
          // if($jml_brg_u != $jml_brg_mas){
          //   date_default_timezone_set('Asia/Jakarta');
          //   $tghi=date("Y-m-d h:i:sa");
          //   $nm_user=$_SESSION['nm_user'];
          //   $nm_brg_mas=$nm_brg_mas.'a:'.$jml_brg_mas.' e:'.$jml_brg_u;
          //   mysqli_query($conseek,"INSERT INTO file_log VALUES('','Jual Act','$kd_brg','$nm_brg_mas','$tghi','$kd_toko','$nm_user')");
          // }
          // mysqli_close($conmasbrg);mysqli_free_result($cekbrg);unset($cx,$jml_brg_u);
