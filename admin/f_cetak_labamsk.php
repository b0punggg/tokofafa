...
<body class="F4 ">      
  <section class="sheet padding-10mm">  
    <div style="page-break-before: always;">
        ...
        if ($_POST['bulanpi']=="1"){
          //bulan ini
          $bulanpil='AND MONTH(mas_jual_hutang.tgl_jual)='.$endbln.' AND YEAR(mas_jual_hutang.tgl_jual)='.$endyear;
          $judul='Penjualan Bulan Ini';
        }
        if ($_POST['bulanpi']=="2"){
          //bulan lalu  
          $bulanpil="AND mas_jual_hutang.tgl_jual < "."'".$tglcari."'";
          $judul='Penjualan Bulan Lalu';
        }
        if ($_POST['bulanpi']=="3"){
          //semua  
          $bulanpil='';
        ...
        if ($pilihan=='alldata'){
            $cek=mysqli_query($connect,"SELECT mas_jual_hutang.*,pelanggan.nm_pel,pelanggan.al_pel 
              FROM mas_jual_hutang 
              LEFT JOIN pelanggan ON mas_jual_hutang.kd_pel=pelanggan.kd_pel
              WHERE MONTH(mas_jual_hutang.tgl_tran)='$endbln' AND YEAR(mas_jual_hutang.tgl_tran)='$endyear' 
              AND mas_jual_hutang.byr_hutang>0 $bulanpil
              ORDER BY mas_jual_hutang.tgl_tran,mas_jual_hutang.no_fakjual ASC");   
        }else {
          $toko=$_POST['kd_tokolabamsk'];
          $cek=mysqli_query($connect,"SELECT mas_jual_hutang.*,pelanggan.nm_pel,pelanggan.al_pel 
              FROM mas_jual_hutang 
              LEFT JOIN pelanggan ON mas_jual_hutang.kd_pel=pelanggan.kd_pel
              WHERE MONTH(mas_jual_hutang.tgl_tran)='$endbln' AND YEAR(mas_jual_hutang.tgl_tran)='$endyear' AND kd_toko='$toko' AND mas_jual_hutang.byr_hutang>0 $bulanpil
              ORDER BY mas_jual_hutang.tgl_tran,mas_jual_hutang.no_fakjual ASC");   
        }
          if(mysqli_num_rows($cek)>=1){
            ?>
              ...
              <thead>
                ...
                <tr> <td colspan="10" style="text-align: left;font-size: 10pt"><b>Laporan Modal & Laba Piutang <?=$judul ?>
                </b></td></tr>
               <tr style="background-color: lightgrey;">
                    ...
                    <th style="width:12%">MODAL</th>
                    <th style="width:12%">LABA</th>
                    <th style="width:12%">NOMINAL</th>
               </tr> 
              </thead>   
    	        <?php
    	        $no=0;$tot_laba=0;$tot_modal=0;$tot_nom=0;
    	      	while($databay=mysqli_fetch_assoc($cek)){
                    $no++; $tot_modal=$tot_modal+$databay['modal'];               
                    $tot_laba=$tot_laba+$databay['laba']; 
                ...
                <tr>
                    ...
                    <td style="text-align:center;font-size: 8pt"><?php echo $databay['nm_pel'].' d/a '.$databay['al_pel'] ?></td>
    	            <td style="text-align:center;font-size: 8pt"><?php echo gantitgl($databay['tgl_tran']); ?></td>
                    <td style="text-align:right;font-size: 8pt"><?php echo gantitides($databay['modal']); ?></td>
                    <td style="text-align:right;font-size: 8pt"><?php echo gantitides($databay['laba']); ?></td>
                ...
                <th style="text-align:right"><?php echo gantitides($tot_modal) ?></th>
                <th style="text-align:right"><?php echo gantitides($tot_laba) ?></th>
