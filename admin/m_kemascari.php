...
<div class="table-responsive" style="overflow-y:auto;overflow-x: auto;border-style: ridge;border-color: white;height: 343px">
	<table id="table" class="arrow-nav table-hover" style="font-size:9pt;position: sticky;width: 100%">
	    ...
	    if(isset($_POST['search']) && $_POST['search'] == true){ // Jika ada data search yg 
	    	...
	    	//echo $params;
          if ($params=="") {	 
          	  $sql = mysqli_query($connect, "SELECT kemas.no_urut,kemas.nm_sat1,kemas.nm_sat2 FROM kemas
          	  	     ORDER BY kemas.nm_sat2 ASC LIMIT $limit_start, $limit");

	          $sql2 = mysqli_query($connect, "SELECT COUNT(*) AS jumlah FROM kemas ORDER BY nm_sat2");
          }
          else {
	          $sql =mysqli_query($connect, "SELECT kemas.no_urut,kemas.nm_sat1,kemas.nm_sat2 FROM kemas
	          	  WHERE kemas.nm_sat2 LIKE '$param'  ORDER BY kemas.nm_sat2 ASC LIMIT $limit_start, $limit");
		      $sql2 = mysqli_query($connect, "SELECT COUNT(*) AS jumlah FROM kemas WHERE nm_sat2 LIKE '$param' ");	
          }	
	      $get_jumlah = mysqli_fetch_array($sql2);

	    }else{ // Jika user belum mengklik tombol search (PROSES TANPA AJAX)
	      // $id_apt=$_SESSION['id_apt'];
          $sql = mysqli_query($connect, "SELECT * from kemas ORDER BY kemas.nm_sat2 ASC LIMIT $limit_start, $limit");
	    ...
	    while($data = mysqli_fetch_array($sql)){ // Ambil semua data dari hasil eksekusi $sql
	      // $no++;
	      $id=mysqli_escape_string($connect,$data['nm_sat2']);
	      if($id<>'-NONE-'){
	        $no++;	
	    ?>
	      <tr>
	        ...
	        <td align="left"><input class="w3-input" type="text" value="<?php echo $data['nm_sat1']; ?>" readonly style="border: none;background-color: transparent;" ></td>
	        <td align="left"><input class="w3-input" type="text" value="<?php echo $data['nm_sat2']; ?>" readonly style="border: none;background-color: transparent;"></td>
	        <td>
	          	<button onclick="
	          	   document.getElementById('nm_sat1').value='<?=mysqli_escape_string($connect,$data['nm_sat1']) ?>';
	          	   document.getElementById('nm_sat2').value='<?=mysqli_escape_string($connect,$data['nm_sat2']) ?>';
	          	   document.getElementById('no_urut').value='<?=mysqli_escape_string($connect,$data['no_urut']) ?>';
	        	   document.getElementById('keyktkemas').value='';
	        	   document.getElementById('btn-ktkemas').click()" class="btn-primary fa fa-edit" style="cursor: pointer; font-size: 12pt" title="Edit Data">
	            </button>	    
	        </td>
	        <td>  
	           <?php $param=mysqli_escape_string($connect,$data['no_urut']); ?>
	           <button onclick="if(confirm('Apakah anda yakin ingin menghapus data ini ??')){ location.href='m_kemashapus_act.php?param=<?php echo $param ?>'}" class="btn-danger fa fa-trash" style="cursor: pointer;font-size: 12pt" title="Hapus Data"></button>
