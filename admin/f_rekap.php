...
<style>
  ...
  while ($datko=mysqli_fetch_assoc($cektoko)) {
    ...
    $wer=mysqli_query($connect,"SELECT * FROM beli_brg WHERE MONTH(tgl_fak)='$endbln' AND YEAR(tgl_fak)='$endyear' AND kd_toko='$tokocari'");
    ...
    $de = mysqli_query($connect,"SELECT mutasi_adj.* FROM mutasi_adj
        WHERE mutasi_adj.kd_toko='$tokocari' AND MONTH(mutasi_adj.tgl_input) ='$endbln' AND YEAR(mutasi_adj.tgl_input) ='$endyear' AND UPPER(mutasi_adj.ket) LIKE '%LINE%' ORDER BY kd_brg ASC");
    ...
    while($edi=mysqli_fetch_assoc($de)){
      ...
      //cari di beli_brg
      $crsq=mysqli_query($connect,"SELECT kd_sat,hrg_beli,kd_brg,disc1,disc2,ppn,jml_brg,no_urut,no_item FROM beli_brg WHERE kd_brg='$xkdbrg' AND kd_toko='$tokocari' AND MONTH(tgl_fak)='$endbln' AND YEAR(tgl_fak)='$endyear' ORDER BY no_urut DESC");
      $noitem=$urut=$xsisa=0;
    ...
    $wer=mysqli_query($connect,"SELECT beli_brg.tgl_fak,beli_brg.no_item_del FROM beli_brg 
        WHERE MONTH(beli_brg.tgl_fak)='$endbln' AND YEAR(beli_brg.tgl_fak)='$endyear' AND beli_brg.kd_toko='$tokocari'");
    $m_ol_t=$m_ol_h=0;
    ...
    while($dbeli=mysqli_fetch_assoc($cbeli)){
      ...
      $totbeli=($totbeli_h+$totbeli_t)-$m_ol_t;
    }
    unset($cbeli,$dbeli); 
    ...
    while($dataom=mysqli_fetch_assoc($cekom)){
        ...
        $jumlah=$jumlah+($dataom['tot_jual']-$dataom['tot_disc']);
        $jual=$jumlah; // total omset       
...
<?php 
 ...
 function cartbag($id_bag,$tgl1,$tgl2,$hub){
  ...
  WHERE MONTH(dum_jual.tgl_jual)='$tgl1' AND YEAR(dum_jual.tgl_jual)='$tgl2' AND dum_jual.id_bag='$id_bag' AND dum_jual.panding='0'
  ...
  while($dtl=mysqli_fetch_assoc($stls)){
    ...
    // $jumbag=($dtl['hrg_jual'])*$dtl['qty_brg'];
    $tot=$tot+$jumbag;
  }
