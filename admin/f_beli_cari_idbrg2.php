...
<div class="table-responsive" style="overflow-y:auto;overflow-x: auto;border-style: ridge;" tabindex="2">
	<table class="hrf_res arrow-nav table-bordered table-striped table-hover" style=" width: 100%">
	<tr align="middle" class="yz-theme-l3">
		<th style="width: 100%">ID BARANG</th>
		<th>OPSI</th>
	</tr>
	<?php
	session_start(); 
	...
	if(isset($_POST['search']) && $_POST['search'] == true){ // Jika ada data search yg 
		...
		if ($params=="") {	 
			...
			$sql1 =mysqli_query($connect, "SELECT * FROM mas_brg 
			WHERE kd_brg LIKE '$param' ORDER BY kd_brg ASC LIMIT $limit_start, $limit");
		...
		$sql1 = mysqli_query($connect, "SELECT * from mas_brg  ORDER BY kd_brg ASC LIMIT $limit_start, $limit");
	...
	?>
		<tr>
		<td align="left" style="cursor: pointer" onclick="document.getElementById('<?='btnidbrg2'.$no?>').click()" ><input class="w3-input" type="text" tabindex="5" style="border: none;background-color: transparent;cursor: pointer" value="<?=$databrg['kd_brg']; ?>" onkeydown="if(event.keyCode==13){document.getElementById('<?='btnidbrg2'.$no?>').click()}" readonly></td>
		<td>
			<button id="<?='btnidbrg2'.$no?>" onclick="
				...
				document.getElementById('boxidbrg').style.display='none';" class="btn btn-sm btn-primary fa fa-edit" type="button" style="cursor: pointer;font-size: 12pt" title="Edit Data" >
			</button>
		</td>    
		</tr>
	<?php
	...
	?><script>
		...
		// document.getElementById('nm_kat').value='';
	...
	<nav  aria-label="Page navigation example " >
		<ul class="pagination justify-content-center hrf_res w3-margin-top">
		<!-- LINK FIRST AND PREV -->
		<?php
		...
		?>
			<li><a class="page-link yz-theme-d1" style="cursor: pointer" href="javascript:void(0);" onclick="cariidbrg2(1, false)">First</a></li>
			<li><a class="page-link yz-theme-l1" style="cursor: pointer;padding-left:17px;padding-right:17px" href="javascript:void(0);" onclick="cariidbrg2(<?php echo $link_prev; ?>, false)"><i class="fa fa-caret-left"></i></a></li>
		<?php
		}
		?>
		...
		?>
			<li class="page-item " <?php echo $link_active; ?>><a class="page-link  yz-theme-l3" href="javascript:void(0);" style="cursor: pointer" onclick="cariidbrg2(<?php echo $i; ?>, false)"><?php echo $i; ?></a></li>
		<?php
		}
		?>
		...
		?>
			<li class="page-item"><a class="page-link yz-theme-l1" href="javascript:void(0)" onclick="cariidbrg2(<?php echo $link_next; ?>, false)" style="cursor: pointer;padding-left:17px;padding-right:17px"><i class="fa fa-caret-right"></i></a></li>
			<li class="page-item "><a class="page-link yz-theme-d1" href="javascript:void(0)" onclick="cariidbrg2(<?php echo $jumlah_page; ?>, false)" style="cursor: pointer">Last</a></li>
		<?php
		}
		?>
		</ul>
